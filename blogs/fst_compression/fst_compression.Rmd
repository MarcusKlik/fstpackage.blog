...
for (level in 10 * 0:5) {
  for (threads in 1:20) {
    ...
    cat(".")
    threads_fst(threads)
    compress_time <- microbenchmark(compressed_vec <- compress_fst(raw_vec, "ZSTD", level), times = 2)
    decompress_time <- microbenchmark(decompress_fst(compressed_vec), times = 2)
